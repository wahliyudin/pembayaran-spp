<div class="px-4 py-8 w-full space-y-4">
    <div class="bg-white shadow px-4 py-4 w-full border-t-2 border-gray-700 rounded">
        <div class="flex flex-col">
            <span class="pb-4 font-semibold">Tarif - {{ $type_of_payment->type_payment }}
                {{ $this->filter['data_class_id'] }}</span>
            ...
            <div class="grid grid-cols-3 gap-6 border-t border-gray-700 py-4">
                <div class="flex space-x-4 items-center">
                    ...
                    <input type="text" class="rounded text-sm bg-gray-300"
                        value="{{ $type_of_payment->schoolYear->school_year }}" readonly placeholder="tahun">
                </div>
                <div class="flex space-x-4 items-center">
    ...
    <div class="bg-white shadow px-4 py-4 w-full border-t-2 border-gray-700 rounded">
        <div class="flex flex-col">
                            ...
                            <tbody>
                                @foreach ($billing_rates as $item)
                                    <tr class="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
                                        ...
                                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                            Rp. {{ number_format($item->billing, 0, ',', '.') }}
                                        </td>
                                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
            ...
            <div class="flex">
                {{ $billing_rates->links('vendor.livewire.tailwind') }}
            </div>
        </div>
    </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div class="flex flex-col space-y-4 border-t-2 border-red-700 rounded shadow px-4 py-4">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <input type="text" class="text-sm bg-gray-300 rounded" readonly
                            value="{{ $type_of_payment->type_payment }}">
                    </div>
                    <div class="flex justify-between items-center">
                        <label for="" class="font-semibold">Tahun Pelajaran</label>
                        <input type="text" class="text-sm bg-gray-300 rounded" readonly
                            value="{{ $type_of_payment->schoolYear->school_year }}">
                    </div>
                    <div class="flex justify-between items-center">
                ...
                <div class="flex flex-col space-y-4 border-t-2 border-gray-700 rounded shadow px-4 py-4">
                    ...
                    <div class="flex justify-between items-center">
                        <label for="" class="font-semibold">Kelas</label>
                        <select wire:model='data_class_id' id="" {{ isset($billingRate) ? 'disabled' : '' }}
                    ...
                    @if (isset($students))
                        <div class="flex justify-between items-center">
                            ...
                            <select wire:model='student_id' id="" {{ isset($billingRate) ? 'disabled' : '' }}
                                class="rounded py-2 px-2">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <input type="number" class="text-sm rounded py-2 px-2" wire:model="billing">
                        @error('billing')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
